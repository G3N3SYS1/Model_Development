...
#             target_categories = np.argmax(outputs[0].cpu().data.numpy(), axis=-1)
            if self.task == 'od':
                ...
                print(f"masks shape: {masks.shape}")# Predicted masks tensor
                target_categories = masks.sum(dim=(2, 3)).argmax(dim=1)
                #target_categories = masks.sum(dim=(1, 2)).argmax(dim=0)
